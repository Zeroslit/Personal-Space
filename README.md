...
- 每次打开「新建项目 / 编辑项目」都是一份全新的表单：上一轮自动填进去的简介不会残留到这一轮，地址被清空或改成别的仓库后旧建议立即作废；你亲手写过或手动清空过简介时，之后也不再自动回填；
...
前端另有一层 vitest 单测（链接规则、表单校验、简介自动填充规则，以及「关掉再打开不残留简介」的表单回归），不需要后端也能跑：
